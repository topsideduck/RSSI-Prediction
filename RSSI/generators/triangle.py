...
def generate_rssi_data(
    ...
    """
    Generate the data set of RSSI values in a triangular room for a given transmitter coordinate.
    ...
    # Create a grid of x and y coordinates in the triangular room
    x = np.arange(0, TRIANGLE_BASE + resolution, resolution)
    y = np.arange(0, TRIANGLE_HEIGHT + resolution, resolution)
    ...
    rssi_data = calculate_rssi_recursive(
        distance_transmitter_to_point, TRIANGLE_HEIGHT, MAX_REFLECTIONS
    )
...
def generate_dataset(
    ...
    # Generate random sample locations within the triangular room
    x_samples = np.random.uniform(0, TRIANGLE_BASE, num_samples)
    y_samples = np.random.uniform(0, TRIANGLE_HEIGHT, num_samples)
    ...
    # Calculate the RSSI values using the recursive approach for multiple reflections
    rssi_values = calculate_rssi_recursive(distances, TRIANGLE_HEIGHT, MAX_REFLECTIONS)
...
if __name__ == "__main__":
    ...
    dataset.to_csv("data/rssi_dataset_triangle.csv", index=False)

    print("Dataset saved to 'data/rssi_dataset_triangle.csv'.")
