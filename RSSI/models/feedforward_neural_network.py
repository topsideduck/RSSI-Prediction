...
class FeedForwardNeuralNetworkModel:
    ...
    def train_model(
        ...
        """Create and train the Feedforward Neural Networks (FNN) model.
        ...
        Returns:
            keras.Model: The trained FNN model.
        """
    ...
    def evaluate_model(
        ...
        """Evaluate the FNN model on the test set and calculate the R-squared (accuracy) of the model.

        Parameters:
            model (keras.Model): The trained FNN model.
...
if __name__ == "__main__":
    # Load the dataset
    feed_forward_neural_network_model = FeedForwardNeuralNetworkModel()
    dataset = feed_forward_neural_network_model.load_dataset(
        "data/rssi_dataset_rectangle.csv"
    )

    # Preprocess the dataset
    X, y = feed_forward_neural_network_model.preprocess_dataset(dataset)
    ...
    trained_model = feed_forward_neural_network_model.train_model(
        X_train, y_train, epochs=100, batch_size=32
    )

    # Evaluate the model
    accuracy = feed_forward_neural_network_model.evaluate_model(trained_model, X_test, y_test)
    print("R-squared (Accuracy):", accuracy)

    # Plot the predictions
    feed_forward_neural_network_model.plot_predictions(
